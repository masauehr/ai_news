---
layout: compare
title: モデル比較（8/22〜8/29）
---

<div class="compare-header">
  <h1>🔬 モデル比較（8/22〜8/29）</h1>
  <div class="compare-meta">
    <span class="badge ollama">🖥️ Ollama</span> <span style="font-family:monospace;font-size:0.82rem;color:#666">qwen3.6:35b-mlx（土曜 09:00 生成）</span>
...
- **🖥️ Ollama**:
  - Anthropic の科学者支援プログラム拡大（研究者1万人へのClaude提供、AI for Science）
  - Model Hardware Standard（MHS）のリサーチプレビュー
  - Gemini Robotics 2（全身制御VLA・マルチロボット協調）
  - Anthropic のテキスト透かし（SynthID-Text、EU AI Act対応）
  - Dario Amodei のオープンウェイト方針表明
  - Hugging Face Blog の abliteration / EdgeFirst Model Zoo
  - Note記事（Galileo AI、jMatsuzaki のプログラマー活用まとめ）
  - Apple TV / Apple One の米国値上げ
  - NVIDIA Cosmos とフィジカルAI（東証プライム企業動向）

- **⚡ Claude Haiku**:
  - Stripe による OpenRouter 買収完了
  - NII「LLM-jp-4 33B」新シリーズ公開（約332億パラメータ、Apache 2.0）
  - MCP ロードマップ更新（Linux Foundation 傘下 AAIF、5重点領域）
  - Qwen3.8-Flash-Next（125B / 6B活性化）の詳細スペック
  - Ollama v0.30/0.32 による 20–30B モデルの実運用（RTX 3060 / 5060 Ti）
  - IDC Japan の生成AI市場規模予測（2024年1,016億円→2028年8,028億円）
  - Google の AI 統括再編（Kavukcuoglu 起用、Hassabis の CEO 退任）
  - Anthropic の金融特化モデル「Claude 3.1 Guardian」
  - Theseus Infrastructure（Macquarie・GIC と共同のデータセンター）、Tino Cuéllar 起用
  - Microsoft Agent Framework 1.0（GA）
  - OpenRouter「Image benchmarks」による画像生成AI比較

- **🦉 ornith**:
  - Gemini 3.5 Transcribe（清書レベル文字起こし）と Gemini Omni 1.1 Flash
  - Hugging Face Blog の Agentic RL 新手法「TITO」
  - OpenAI の強化学習を2週間一時停止（安全体制拡充）
  - OpenAI の PII 非識別化オープンウェイトツール（F1 96%）
  - GLM-5.3 のオープンウェイト公開（Z.AI）
  - BitNet-embedding-270M の追加
  - OpenAI の「常時稼働・自己起動型エージェント」構想（The Decoder）
  - Claude Code の高速化（デスクトップ起動2倍、CLIのCPU使用率半減）、Claude からの Gmail 直接送信
  - SB Intuitions「Sarashina3」、PFN「PLaMo 2.1 Prime」

- **🌩️ nemotron**:
  - OpenAI が ChatGPT 無料版を無制限化、「GPT-5.6 Luna」公開
  - Apple の AI 常時稼働 Mac mini（14.9万円）発表

---

### 各観点の評価

| 観点 | **🖥️ Ollama**<br>qwen3.6:35b-mlx | **⚡ Claude Haiku**<br>claude-haiku-4-5 | **🦉 ornith**<br>ornith-1.5:35b | **🌩️ nemotron**<br>nemotron-3.5-lightning:30b-mlx |
|------|------|------|------|------|
| **情報の深さ** | ⭐×4 ベンチマーク数値や背景を各トピックに添え、11項目を厚く記述。ただし時期の混乱を自ら注記する場面も | ⭐×4 日付・固有名詞・数値が最も豊富で深いが、脱線と古い話題の混在で密度にムラ | ⭐×3 各トピック2段落程度で具体性はそこそこ。深掘りは中庸 | ⭐×2 日付・内容・URL・備考の短文のみで、各項目の踏み込みが浅い |
| **カバレッジ** | ⭐×5 M&A・モデル・規制・透かし・HF Blog・Note・エージェントまで最広級 | ⭐×5 10本＋サブトピックで市場・投資・画像生成まで網羅、最も広い | ⭐×3 主要8トピックは押さえるがNote記事や規制報道が手薄 | ⭐×2 基本7トピックのみでMCP・セキュリティ・透かし等が欠落 |
| **国内AI動向** | ⭐×4 源内＋国産7モデル列挙＋調達検討会＋日本人著者Note＋日本企業フィジカルAI | ⭐×5 LLM-jp-4 33B・源内18万人実証・IDC市場・企業導入・国産ロボ44社と最も厚い | ⭐×4 源内＋7モデル（NTTデータ/NEC/富士通/PFN）＋Sarashina3＋PLaMo。ただし一部は数週間前の話題 | ⭐×2 源内とLLM-jp-4採用の1トピックにとどまる |
| **読みやすさ** | ⭐×3 見出し構造は明瞭だが「透波」「オープンウェights」等の誤変換が散見 | ⭐×4 太字サブ見出しと参考リンクで整理され追いやすい | ⭐×4 導入引用・トピック一覧表・まとめが揃い最も整然 | ⭐×3 定型フォーマットで簡潔だが単調で味気ない |
| **情報源の明示** | ⭐×4 各トピックに複数URL。一部の一般論セクションはリンクが薄い | ⭐×3 各セクション「参考」1本のみで、複数話題を1URLに束ね出典対応が粗い | ⭐×5 文中に逐一リンクを挿入し1トピック複数ソース、最も丁寧 | ⭐×3 各項目1URLだがYouTube・個人ブログ依存が目立ち最小限 |
| **ビジネス視点** | ⭐×3 買収戦略・コスト半減・価格改定に言及する程度 | ⭐×5 M&A・市場規模予測・データセンター投資・導入事例と最も充実 | ⭐×3 業界再編とコスト性能バランスへの言及にとどまる | ⭐×2 「無料化」「導入コスト低下」のトレンド指摘のみで軽い |

---

### 総評

今週はNVIDIAによるHugging Face買収報道が全モデル共通の軸で、これにClaude Opus 5 / Sonnet 5、Gemini新モデル、API推論ブロックの脆弱性、デジタル庁「源内」と国産LLMが続く構図だった。⚡Haikuは市場規模・データセンター投資・企業導入まで踏み込み、ビジネス視点と国内動向が最も厚いが、GPT-4.5やGemini 2など古い情報の混在と、1セクション1リンクで出典対応が粗い点が弱み。🖥️Ollamaはカバレッジが広く、透かし・規制・HF Blog・Note記事まで拾うが、「透波」等の誤変換とリリース時期の混乱が目立つ。🦉ornithは逐一リンクを付す出典明示が随一で、一覧表と導入文により可読性も高いが、Sarashina3など数週間前の話題が混じり深掘りは中程度。🌩️nemotronは定型フォーマットで簡潔だが7トピックと最も浅く、YouTubeを出典にするなど信頼性に難がある。ローカル系3モデルを比べると、Ollamaが網羅性、ornithが出典の丁寧さと構成、nemotronが速報的な簡潔さと三者三様で、各記事を合わせて読むことで今週の全体像と各モデルの得手不得手が浮かび上がる。
